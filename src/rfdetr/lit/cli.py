...
Both ``RFDETRModule`` and ``RFDETRDataModule`` share the same
...
def main() -> None:
    ...
    Constructs and runs ``RFDETRCli`` with ``RFDETRModule`` and
    ``RFDETRDataModule``.  Subcommands are auto-generated by
    ``LightningCLI``: ``fit``, ``validate``, ``test``, ``predict``.
    """
    RFDETRCli(RFDETRModule, RFDETRDataModule)
