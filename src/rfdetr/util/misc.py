...
def _dist_tensor_device() -> torch.device:
    """Return a safe device for temporary distributed helper tensors."""
    if torch.cuda.is_available() and dist.is_available() and dist.is_initialized() and dist.get_backend() == "nccl":
...
class SmoothedValue(object):
    ...
    def synchronize_between_processes(self) -> None:
        ...
        if not is_dist_avail_and_initialized():
            return
        t = torch.tensor([self.count, self.total], dtype=torch.float64, device=_dist_tensor_device())
        dist.barrier()
        dist.all_reduce(t)
        t = t.tolist()
        self.count = int(t[0])
